...
import { isPatchFailureResult } from "../../lib/patchFile";
...
const TOOL_ICONS: Record<string, React.ReactNode> = {
  ...
  apply_site_theme: <Wrench size={14} className="text-violet-400" />,
...
export const ToolCallCard = memo(function ToolCallCard({
  ...
  const [expanded, setExpanded] = useState(
    () => toolName === "patch_file" && !!result && isPatchFailureResult(result),
  );
  const patchFailed = toolName === "patch_file" && result && isPatchFailureResult(result);
  const patchSummary = patchFailed && result
    ? (/Result: (\d+\/\d+) patches applied/.exec(result)?.[1] ?? "failed")
    : null;
  const isSuccess =
    result &&
    !patchFailed &&
    (result.startsWith("OK") || result.includes("✓"));
  const isError =
    result &&
    (result.startsWith("Error") ||
      result.includes("✗") ||
      patchFailed ||
      result.includes("PATCH_FAILED: true"));
            ...
            <div className="space-y-2">
              {patchFailed && (
                <p className="text-xs font-medium text-red-500">
                  Patch failed — file was not updated. Full file context is included below for write_file.
                </p>
              )}
              <pre className="text-xs font-mono whitespace-pre-wrap text-muted-foreground leading-relaxed max-h-96 overflow-y-auto">
                {result}
              </pre>
            </div>
          ) : (
