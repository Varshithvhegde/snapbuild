...
/** Default model when switching to a provider preset (if applicable). */
export const DEFAULT_MODELS: Partial<Record<ApiType, string>> = {
  inception: "mercury-2",
};

/** Models known to a provider when /models cannot be fetched (e.g. CORS). */
export const KNOWN_MODELS: Partial<Record<ApiType, string[]>> = {
  inception: ["mercury-2", "mercury-edit-2"],
};

/** Models / providers that do not support vision — send image URLs as text only. */
export function modelSupportsVision(apiType: ApiType, model: string): boolean {
  if (apiType === "inception") return false;
  const id = model.trim().toLowerCase();
  if (id.startsWith("mercury")) return false;
  return true;
}
...
export async function fetchModelList(
  ...
  switch (apiType) {
    case "inception":
      return [...(KNOWN_MODELS.inception ?? ["mercury-2"])];
